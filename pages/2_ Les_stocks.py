...
infos_stocks = datas_stocks.get_infos_stocks() 

# Indice par défaut pour graph et tableau 
stocks_default = "Apple Inc."
...
default_index = stocks_default # "index=indices.index(default_index)" attent un int pour index
selected_stock = st.selectbox("Choisissez une entreprise pour le graphique", liste_stocks, index=liste_stocks.index(default_index)) # arg1 : nom liste déroulante / arg2 : liste pour la liste déroulante / arg3 : opt par défaut de l'actif pour visualisation graph.

# Récupération des données "Dates" et "Close" de la base de donnée pour le graphique en dataframe
df = datas_stocks.get_prix_date(selected_stock)

# S'il y a des données dans les colonnes, graphique, sinon message d'erreur.
if not df.empty:
    fig = go.Figure(go.Scatter(x=df["Date"], y=df["Close"], mode='lines', name=selected_stock, line=dict(color='#6DBE8C', width=2)))
    fig.update_layout(title=f"Évolution de {selected_stock} - Clôture hebdomadaire", xaxis_title="Date", yaxis_title="Prix de clôture")
    st.plotly_chart(fig)
else:
...
selected_stock = stocks_default 
stock_selectionner_pour_tableau = st.multiselect("Ajoutez une entreprise au tableau pour comparer", liste_stocks, default= [stocks_default]) # arg1 : nom liste déroulante / arg2 : liste pour la liste déroulante / arg3 : opt par défaut de l'actif sur le tableau. "default= [default_indices]" entre [] car attend une liste.
...
for i in st.session_state.rendement_data.index:
    if i not in stock_selectionner_pour_tableau:
...
for i in stock_a_ajouter:
    df_prix_date = datas_stocks.get_prix_date(i) # On crée le df avec en colonne "Date" et "Close" pour chaque indice selectionnés ds "indices_to_add"
    if not df.empty:
        df_rendement = calculate_rendement(df_prix_date, periods)
        df_info = infos_stocks[infos_stocks["Short_Name_Stocks"] == i]  # Filtres les infos sur l'indice
        
        if not df_info.empty:
            df_rendement["Pays_Stocks"] = df_info.iloc[0]["Pays_Stocks"]
            df_rendement["Ticker_Stocks"] = df_info.iloc[0]["Ticker_Stocks"]
            df_rendement["Ticker_Stocks_Yf"] = df_info.iloc[0]["Ticker_Stocks_Yf"]
            df_rendement["Place_Boursiere"] = df_info.iloc[0]["Place_Boursiere"]
        else:
            df_rendement["Pays_Stocks"] = "Inconnu"
...
st.session_state.rendement_data = st.session_state.rendement_data[["Pays_Stocks"] + [f"{p} mois" for p in periods] + ["Ticker_Stocks", "Ticker_Stocks_Yf", "Place_Boursiere"]]
