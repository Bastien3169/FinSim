...
from test_def_app import connect_to_db, get_list_actif, get_infos_actif,  get_hist_actif_for_graph, calculate_rendement, style_rendement
...
liste_indices = get_list_actif(table_hist_actif)
df_infos_indices = get_infos_actif(table_infos_actif)
...
if response.status_code == 200:
    ...
    df = pd.DataFrame() # Création d'un DataFrame vide en cas d'erreur
...
df = get_hist_actif_for_graph(table_hist_actif, selected_indice)
...
for i in indices_to_add:
    df = get_hist_actif_for_graph(table_hist_actif, i) # On crée le df avec en colonne "Date" et "Close" pour chaque indice selectionnés ds "indices_to_add"
    if not df.empty:
