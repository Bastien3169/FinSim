...
st.markdown(f"""<div class="main-container"><h1>👑 ADMINISTRATEUR : MISE À JOUR DES BDD</h1></div>""",
...
for i in admin_manager.get_all_users():
    if i[3] == 'admin':  # Si on trouve un admin en username ! Changer par i[3] pour recherchepar rôle !!!
        admin_exists = True
...
# Si 'admin' existe pas, création bouton unique pour créer admin
    ...
    if st.button("Créer l'utilisateur admin initial"):
        auth_manager.register('admin', 'jolie.mountain@gmail.com', 'Admin#1')
...
############################################### FOOTER ###############################################
st.markdown("""<div class="footer"> © 2025 Bastien M. - Projet finance — Tous droits réservés.</div>""",
            unsafe_allow_html=True)
